...
{
    public class FancyMagicMoves : SlidingMoves
    {
        static FancyMagicMoves()
        {
            ClassicMoves classic = new();
            ...
            for (int x = 0; x < 8; x++)
            {
                for (int y = 0; y < 8; y++)
                {
                    ...
                    for (ulong i = 0; i < (ulong)(1 << cnt); i++)
                    {
                        ...
                        fancyLookupTable[FancyRookIndex(sq, blockers)] = classic.GetRookAttacks(sq, blockers);
                    }
                    ...
                    for (ulong i = 0; i < (ulong)(1 << cnt); i++)
                    {
                        ...
                        fancyLookupTable[FancyBishopIndex(sq, blockers)] = classic.GetBishopAttacks(sq, blockers);
                    }
                }
            }
        }

        internal readonly struct FancyHash
        {
        ...
        {
            ref FancyHash m = ref fancyRookMagics[square];
        ...
        {
            ref FancyHash m = ref fancyBishopMagics[square];
            return m.Offset + (int)(((blockers | m.Mask) * m.Hash) >> (64 - 9));
        }

        private static readonly UnsafeArray<ulong> fancyLookupTable = new(88507);
